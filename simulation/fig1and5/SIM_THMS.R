...
library(data.table)
#library(doParallel)
reg = makeExperimentRegistry(file.dir = 'yourdirectory', seed = 1)

#set.seed(1026)
make_data = function(data, job, d = 50, ord = 1){
  library(pracma)
  library(MASS)
  ...
  list(V = V, pmi_true = pmi_true, pmi_lr_true = pmi_lr_true, d = d, ord = ord)
}
addProblem(name="normaldata",fun=make_data)

pmisimu <- function(data, job, instance, n, c_len){
  library(pracma)
  library(MASS)
###########   functions  #############
data_gene <- function(tm,p,alpha,V){
...
#set.seed(1)
d <- instance$d                                                            #number of words
ord <- instance$ord
V <- instance$V
pmi_true <- instance$pmi_true
pmi_lr_true <- instance$pmi_lr_true
#n <- 200
p <- floor(log(d)^2) + 1                                           #dimension of word vector
q <- 5                                                             #window size
g = 1
#c_len <- 200                                                       #length of time series
f = 10
alpha <- 1 - log(d)/(p^2)
T1 = c_len*q - q*(q+1)/2
vec_p = apply(V, 1, function(x) exp(sum(x^2)/(2*p)))
vec_p = vec_p/sum(vec_p)
alpha_p = 2*sum(sapply(1:q, function(u) (c_len - u)*alpha^{u}))/(p*(2*q*c_len - q^2 -q))
...
for(k in 1:m){
  total_seq <- matrix(0, nrow = n, ncol = c_len)
  for(l in 1:n){                                                   
    total_seq[l,] <- data_gene(c_len,p,alpha,V_hat)
  }
  co_etr <- matrix(0,d,d)
  for (j in 1:n) {
    tmp2 <- total_seq[j,] 
  ...
  SP_est_lr <- SPPMI_calc_lr(co_etr, q, d, c_len, n, p)
  pmi_bt[k,1:f] <- SP_est_lr[g,1:f]
}
sd_PMI_entry_lr <- apply(pmi_bt, MARGIN = 2, sd)
...
for(k in 1:m){
  ...
  for(j in 1:f){
    ...
    if (abs(PMI_entry_list_37[k,j] - pmi_lr_true[g,j]) <= sd_PMI_entry_lr[j]*qnorm(0.975)){
      cover_prob_lr_2[j] <- cover_prob_lr_2[j]+1
    }
...
cover_prob_lr_2 <- cover_prob_lr_2/m
...
################save results###############
save(V, VVT, PMI_mean_est, PMI_entry_sd_co, PMI_entry_sd_lr_co, sd_PMI_entry_lr, PMI_entry_list_36, PMI_entry_list_37,cover_prob, cover_prob_lr, cover_prob_lr_2, cover_prob_3, cover_prob_lr_3, cover_prob_4, cover_prob_lr_4, pmi_true, pmi_lr_true, file=paste0('1102CheckNormal_d',d,'_n',n,'_clen',c_len,'_ord',abs(ord),'.Rdata'))
}
addAlgorithm(name = "pmisimulation", fun = pmisimu)

prob_design = list("normaldata" = CJ(d = c(50,100), ord = c(1,2)))
algo_design = list("pmisimulation" = CJ(n = c(200,400,600,800,1000), c_len = c(400,600,800,1000,1200)))

addExperiments(prob_design, algo_design, repls = 1)

resources = list(account = "youraccount", walltime = '60:00:00', memory='128g', ncpus=1)

submitJobs(data.table(job.id=1:100,chunk=1:25),resources)
